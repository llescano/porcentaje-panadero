...
class Ingredient {
  ...
  bool contributesToHydration; // Nueva propiedad para indicar si contribuye a la hidratación
  double? conversionFactorToGrams; // Nueva propiedad para el factor de conversión manual a gramos
  bool isActive;
  DateTime createdAt;
  DateTime updatedAt;
  ...
  factory Ingredient.fromMap(Map<String, dynamic> map) {
    return Ingredient(
      ...
      defaultUnit: MeasurementUnit.values[map['default_unit'] as int? ?? MeasurementUnit.grams.index], // Valor por defecto si es nulo
      costPerUnit: map['cost_per_unit'] != null ? Decimal.parse(map['cost_per_unit'].toString()) : null,
      supplier: map['supplier'] as String?,
      expirationDate: map['expiration_date'] != null ? DateTime.parse(map['expiration_date'] as String) : null,
      category: IngredientCategory.values[map['category'] as int? ?? IngredientCategory.other.index], // Valor por defecto si es nulo
      contributesToHydration: (map['contributes_to_hydration'] as int? ?? 0) == 1, // Valor por defecto si es nulo
      conversionFactorToGrams: map['conversion_factor_to_grams'] as double?, // Deserializar la nueva propiedad
      isActive: (map['is_active'] as int? ?? 1) == 1, // Valor por defecto si es nulo
  ...
  Map<String, dynamic> toMap() {
    return {
      ...
      'conversion_factor_to_grams': conversionFactorToGrams, // Serializar la nueva propiedad
  ...
  Ingredient copyWith({
    ...
    final ingredient = Ingredient(
      ...
      contributesToHydration: contributesToHydration ?? this.contributesToHydration,
      conversionFactorToGrams: conversionFactorToGrams ?? this.conversionFactorToGrams, // Asignar en copyWith
