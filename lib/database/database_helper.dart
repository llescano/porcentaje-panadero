...
class DatabaseHelper {
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    ...
    if (oldVersion < 2 && newVersion >= 2) {
      ...
      await db.execute('ALTER TABLE recipe_ingredients ADD COLUMN sort_order INTEGER NOT NULL DEFAULT 0');
    }
    if (oldVersion < 3 && newVersion >= 3) {
      // Migración de v2 a v3: añadir created_at y updated_at a recipe_ingredients
      await db.execute('ALTER TABLE recipe_ingredients ADD COLUMN created_at TEXT NOT NULL DEFAULT (strftime(\'%Y-%m-%dT%H:%M:%S.%fZ\', \'now\'))');
      await db.execute('ALTER TABLE recipe_ingredients ADD COLUMN updated_at TEXT NOT NULL DEFAULT (strftime(\'%Y-%m-%dT%H:%M:%S.%fZ\', \'now\'))');
    }
    if (oldVersion < 4 && newVersion >= 4) {
      // Migración de v3 a v4: añadir contributes_to_hydration a recipe_ingredients
      await db.execute('ALTER TABLE recipe_ingredients ADD COLUMN contributes_to_hydration INTEGER NOT NULL DEFAULT 0');
    }
    if (oldVersion < 5 && newVersion >= 5) {
      // Migración de v4 a v5: añadir contributes_to_hydration a ingredients
      await db.execute('ALTER TABLE ingredients ADD COLUMN contributes_to_hydration BOOLEAN NOT NULL DEFAULT 0;');
    }
    ...
    if (oldVersion < 8 && newVersion >= 8) {
      ...
      await db.execute('ALTER TABLE ingredients ADD COLUMN conversion_factor_to_grams REAL;');
    }
    if (oldVersion < 9 && newVersion >= 9) {
      // Migración de v8 a v9: añadir display_unit a recipe_ingredients
      await db.execute('ALTER TABLE recipe_ingredients ADD COLUMN display_unit INTEGER;');
    }
  }
    ...
    final basicIngredients = [
      {
        ...
        'default_unit': MeasurementUnit.grams.index, // Cambiado de 'unit' a 'default_unit'
      ...
      },
      {
        ...
        'default_unit': MeasurementUnit.milliliters.index, // Cambiado de 'unit' a 'default_unit'
      ...
      },
      {
        ...
        'default_unit': MeasurementUnit.pieces.index, // Cambiado de 'unit' a 'default_unit'
